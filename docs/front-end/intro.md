...
We're moving to Tailwind and shadcn for styling — see [Styling with Tailwind and shadcn](./Styling-with-shadcn-and-Tailwind.md) for how this works and when to use it. New UI should be built this way; older components still use SCSS (`src/assets/styles/`), and we recommend using the `search codebase for text` feature of your IDE to find where those older styles are defined until they're migrated.
